...
class TestPromptCache(unittest.TestCase):
    ...
    def test_turboquant_delegates_to_make_cache_for_hybrid(self):
        """kv_cache_type=turboquant + make_cache must not wrap SSM ArraysCache in TurboQuant."""

        class _HybridStub:
            def make_cache(self):
                return [ArraysCache(size=2), KVCache(), ArraysCache(size=2), KVCache()]

        caches = make_prompt_cache(_HybridStub(), kv_cache_type="turboquant")
        self.assertEqual(len(caches), 4)
        self.assertIsInstance(caches[0], ArraysCache)
        self.assertIsInstance(caches[1], KVCache)
        self.assertIsInstance(caches[2], ArraysCache)
