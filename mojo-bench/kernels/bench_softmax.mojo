...
from harness.stats import sort_float_list, percentile, compute_mean, compute_std, percentile_bootstrap_ci, durbin_watson
from harness.output import write_kernel_result_json
...
def bench_softmax_shape(
    ...
    """Benchmark softmax for given sequence length. Returns per-iteration timings in microseconds."""
    ...
    var times = List[Float64](capacity=ITERS)
    for _ in range(ITERS):
        ...
        times.append(Float64(t1 - t0) / 1000.0)  # nanoseconds to microseconds
...
def main() raises:
    comptime if not has_accelerator():
        ...
        for i in range(len(seq_lengths)):
            ...
            var times = bench_softmax_shape(ctx, S, name)
            if len(times) == 0:
                print()
                continue

            # Compute DW on execution-order data BEFORE sorting
            var dw = durbin_watson(times)

            # Compute stats using shared harness
            sort_float_list(times)
            var mean = compute_mean(times)
            var std = compute_std(times, mean)
            var median = percentile(times, 50.0)
            var p5 = percentile(times, 5.0)
            var p95 = percentile(times, 95.0)
            var p99 = percentile(times, 99.0)
            var ci = percentile_bootstrap_ci(times)
            var ci95_lo = ci[0]
            var ci95_hi = ci[1]

            var gbs = compute_bandwidth(total_elements, 4, median)  # FP32 = 4 bytes

            print(name, ":")
            print("  Shape: (1,", NUM_HEADS, ",", S, ",", S, ")")
            print("  Elements:", total_elements)
            print("  Time:", median, "us (median),", mean, "us (mean)")
            print("  Std:", std, "us, CI95: [", ci95_lo, ",", ci95_hi, "]")
            print("  Bandwidth:", gbs, "GB/s")
            print()

            # Write per-shape JSON result — shape format: BxHxSxS
            var shape_str = "1x" + String(NUM_HEADS) + "x" + String(S) + "x" + String(S)
            write_kernel_result_json(
                framework="mojo",
                framework_version="0.26.3",
                kernel_name="softmax",
                shape_name=shape_str,
                mean_us=mean, median_us=median, std_us=std,
                p5_us=p5, p95_us=p95, p99_us=p99,
                ci95_lo=ci95_lo, ci95_hi=ci95_hi,
                n_iterations=ITERS,
                dw_statistic=dw,
                tflops=0.0, gbs=gbs,
                roofline_pct=0.0,
                output_path="results/softmax_" + name + ".json",
                dtype="float32",
            )
