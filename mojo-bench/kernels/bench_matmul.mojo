...
def bench_matmul_shape(
    ...
    """Benchmark single GEMM shape. Returns per-iteration timings in microseconds."""
...
def main() raises:
    comptime if not has_accelerator():
        ...
        for i in range(len(shapes_m)):
            ...
            var times = bench_matmul_shape(ctx, M, N, K, name)
            if len(times) == 0:
                print()
                continue
            ...
            var tflops = compute_tflops(M, N, K, median)
            ...
            # Write per-shape JSON result — shape format: MxNxK
            var shape_str = String(M) + "x" + String(N) + "x" + String(K)
            write_kernel_result_json(
                ...
                n_iterations=ITERS,
                dw_statistic=dw,
                tflops=tflops, gbs=0.0,
                roofline_pct=roofline_pct,
                output_path="results/matmul_" + name + ".json",
                dtype="float32",
            )
