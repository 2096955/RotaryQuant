...
def match_kernels(
    ...
    Matches on (name, shape) only — dtype mismatches (e.g., MLX float16 vs
    Mojo float32) are expected and flagged in the output tables.
    ...
    for mlx in mlx_results:
        mlx_key = (mlx.name, mlx.shape)
        for mojo in mojo_results:
            mojo_key = (mojo.name, mojo.shape)
            if mlx_key == mojo_key:
...
def generate_latex_tables(
    ...
    for category, pairs in categories.items():
        ...
        lines.append(
            r"Shape & MLX dtype & Mojo dtype & Device & MLX ($\mu$s) & Mojo ($\mu$s) & MLX TFLOPS & Mojo TFLOPS & Speedup \\"
        )
        ...
        for mlx, mojo in sorted(pairs, key=lambda p: p[0].shape):
            ...
            # Flag precision mismatch
            dtype_note = ""
            if mlx.dtype != mojo.dtype:
                dtype_note = " $\\dagger$"

            lines.append(
                f"{mlx.shape} & {mlx.dtype} & {mojo.dtype} & GPU & "
                f"{mlx.median_us:.1f} & {mojo.median_us:.1f} & "
                f"{mlx_tflops:.2f} & {mojo_tflops:.2f} & {speedup:.2f}x{dtype_note} \\\\"
            )

        lines.append(r"\bottomrule")
        lines.append(r"\end{tabular}")
        # Add precision mismatch footnote if any pair has different dtypes
        has_mismatch = any(m.dtype != j.dtype for m, j in pairs)
        if has_mismatch:
            lines.append(
                r"\vspace{0.5em}\footnotesize{$\dagger$ Precision mismatch: speedup not directly comparable.}"
            )
...
def generate_roofline_plot(
    ...
    peak_bw_gbs = roofline_data["calibration"].get("peak_memory_bandwidth_gbs", 400.0)
...
def generate_decode_attribution(
    ...
    return {
        ...
        # Measured end-to-end decode time for Nemotron-H 120B on M4 Max: ~53ms/tok
        # with expert offload. Kernel-sum is a lower bound (excludes residual,
        # LayerNorm, sampling, dispatch overhead between kernels).
        "measured_decode_us": 53000,
        "fraction_of_decode_mlx": min(mlx_total / 53000, 1.0),
        "fraction_of_decode_mojo": "N/A (no end-to-end model)",
...
def write_summary_json(
    ...
    # Detect asymmetric iteration counts (affects Cohen's d statistical power)
    asymmetric_n = []
    for mlx, mojo in matches:
        if mlx.n_iterations != mojo.n_iterations:
            asymmetric_n.append(
                f"{mlx.name}_{mlx.shape}: MLX n={mlx.n_iterations}, Mojo n={mojo.n_iterations}"
            )
    ...
    if asymmetric_n:
        summary["cohens_d_note"] = (
            "Cohen's d uses unequal n (MLX adaptive up to 500, Mojo fixed at 50). "
            "Statistical power is asymmetric — significance claims favour the higher-n side."
        )
        summary["asymmetric_n_pairs"] = asymmetric_n
