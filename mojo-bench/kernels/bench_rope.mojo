...
def bench_rope_shape(
    ...
    """Benchmark RoPE for given sequence length. Returns per-iteration timings in microseconds."""
...
def main() raises:
    comptime if not has_accelerator():
        ...
        for i in range(len(seq_lengths)):
            ...
            var times = bench_rope_shape(ctx, S, name)
            if len(times) == 0:
                print()
                continue
            ...
            var gbs = compute_bandwidth(total_elements, 4, median)
            ...
            # Write per-shape JSON result — shape format: BxHxSxD
            var shape_str = "1x" + String(NUM_HEADS) + "x" + String(S) + "x" + String(HEAD_DIM)
            write_kernel_result_json(
                ...
                tflops=0.0, gbs=gbs,
                roofline_pct=0.0,
                output_path="results/rope_" + name + ".json",
                dtype="float32",
            )
